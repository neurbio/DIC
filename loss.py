...
class GG(object):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            part1 = tf.math.log(y_pred+eps) + tf.math.lgamma(beta_hat+eps) - tf.math.log(gamma_hat+eps) - (beta_hat*gamma_hat-1)*(tf.math.log(y_true+eps)-tf.math.log(y_pred+eps))
...
class TPGG(GG):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            gg_case = super().loss(y_true, y_pred, mean=False) - tf.math.log(1.0-self.pi_hat+eps)

            y_true = tf.cast(y_true, tf.float32)

            zero_case = -tf.math.log(self.pi_hat + eps)
