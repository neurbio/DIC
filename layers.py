...
class SoftKMeansLayer(tf.keras.layers.Layer):
    def __init__(self, n_clusters, **kwargs):
        super(SoftKMeansLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        self.cluster_centers = self.add_weight(name='cluster_centers',
                                               ...
                                               trainable=True)
        super(SoftKMeansLayer, self).build(input_shape)

    def call(self, inputs):
        # Compute distances to cluster centers
        distances = tf.reduce_sum(tf.square(tf.expand_dims(inputs, axis=1) - self.cluster_centers), axis=2)
